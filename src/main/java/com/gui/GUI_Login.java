...
public class GUI_Login extends JPanel implements ActionListener {
    ...
    public GUI_Login() {
        ...
        // Sửa lại ActionListener của txtUsername: Chỉ nhảy sang Pass nếu menu đang đóng
    ...
    private void setupSuggestionMenu() {
        ...
        // Bắt phím Enter ngay trên danh sách gợi ý
        ...
        txtUsername.addKeyListener(new KeyAdapter() {
            @Override
            public void keyPressed(KeyEvent e) {
                // Sử dụng keyPressed thay vì keyReleased để ưu tiên chặn sự kiện Enter
                if (e.getKeyCode() == KeyEvent.VK_ENTER && suggestionMenu.isVisible()) {
                    confirmSelection();
                    e.consume(); // Chặn sự kiện Enter lan ra các Listener khác
                }
            }
    ...
    // Các hàm còn lại (saveToHistory, createPasswordPanel, handleLogin, ...) giữ nguyên như cũ
    // Đảm bảo không thay đổi logic lưu history để tính năng này hoạt động đúng.
