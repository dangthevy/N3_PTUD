...
public class TAB_Dashboard extends JPanel {
    ...
    public TAB_Dashboard(GUI_General parent, String role) {
        ...
        if (this.role.equalsIgnoreCase("QuanLy") || this.role.equalsIgnoreCase("Quản lý") || this.role.equalsIgnoreCase("Admin")) {
            initManagerView();
        } else {
            initStaffView();
        }
    ...
    private void initManagerView() {
        ...
        try (Connection con = ConnectDB.getConnection(); Statement st = con.createStatement()) {
            ResultSet rs1 = st.executeQuery("SELECT ISNULL(SUM(thanhTien), 0) FROM HoaDon WHERE CAST(ngayLap AS DATE) = CAST(GETDATE() AS DATE)");
            if(rs1.next()) tongDoanhThu = rs1.getLong(1);
            
            ResultSet rs2 = st.executeQuery("SELECT COUNT(*) FROM HoaDon WHERE CAST(ngayLap AS DATE) = CAST(GETDATE() AS DATE)");
    ...
    private void initStaffView() {
        ...
        try (Connection con = ConnectDB.getConnection(); Statement st = con.createStatement()) {
            ResultSet rs1 = st.executeQuery("SELECT COUNT(*) FROM HoaDon WHERE CAST(ngayLap AS DATE) = CAST(GETDATE() AS DATE)");
    ...
    private JFreeChart createRevenue7DaysChart() {
        ...
        try (Connection con = ConnectDB.getConnection(); Statement st = con.createStatement()) {
            String sql = "SELECT TOP 7 FORMAT(ngayLap, 'dd/MM') as Ngay, SUM(thanhTien) as DoanhThu FROM HoaDon WHERE ngayLap >= CAST(DATEADD(day, -6, GETDATE()) AS DATE) GROUP BY FORMAT(ngayLap, 'dd/MM'), CAST(ngayLap AS DATE) ORDER BY CAST(ngayLap AS DATE) ASC";
    ...
    private JFreeChart createSeatTypePieChart() {
        ...
        plot.setSectionPaint("Giường nằm K4", C_ORANGE_FG);
        plot.setSectionPaint("Giường nằm K6", C_GREEN_FG);
        return chart;
    }
    ...
    private JFreeChart createRevenueByRouteChart() {
        DefaultCategoryDataset dataset = new DefaultCategoryDataset();
        try (Connection con = ConnectDB.getConnection(); Statement st = con.createStatement()) {
            String sql = "SELECT TOP 5 t.tenTuyen, SUM(cthd.donGia) as DoanhThu FROM ChiTietHoaDon cthd JOIN Ve v ON cthd.maVe = v.maVe JOIN LichTrinh lt ON v.maLT = lt.maLT JOIN ChuyenTau ct ON lt.maChuyen = ct.maChuyen JOIN Tuyen t ON ct.maTuyen = t.maTuyen GROUP BY t.tenTuyen ORDER BY DoanhThu DESC";
            ResultSet rs = st.executeQuery(sql);
            boolean hasData = false;
            while(rs.next()) {
    ...
    // TẠO THẺ KPI HIỆN ĐẠI & NHỎ GỌN HƠN
    private JPanel createModernStatCard(String title, String value, Color bgColor, Color iconColor, String iconTxt, String trend, boolean isUp) {
        JPanel p = new JPanel(new BorderLayout(0, 10)); // Giảm khoảng cách dọc
        p.setBackground(BG_CARD);
        // GIẢM PADDING VIỀN XUNG QUANH TỪ 25 XUỐNG 18
        ...
        lblIcon.setFont(new Font("Segoe UI", Font.BOLD, 14)); // Thu nhỏ font chữ icon
        lblIcon.setForeground(Color.WHITE);
        lblIcon.setOpaque(true);
        lblIcon.setBackground(iconColor);
        lblIcon.setPreferredSize(new Dimension(36, 36)); // Thu nhỏ hình vuông icon
        ...
        lblTrend.setBorder(new EmptyBorder(4, 8, 4, 8)); // Thu nhỏ nhãn trend
        ...
        lblV.setFont(new Font("Segoe UI", Font.BOLD, 22)); // Thu nhỏ số (Từ 28 -> 22)

        JLabel lblT = new JLabel(title); 
        lblT.setForeground(TEXT_MID); 
        lblT.setFont(new Font("Segoe UI", Font.PLAIN, 13)); // Thu nhỏ tiêu đề

        JPanel pnlText = new JPanel(new GridLayout(2, 1, 0, 2)); // Kéo sát 2 dòng chữ lại
