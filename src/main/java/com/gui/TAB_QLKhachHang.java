...
//        lblSuffix.setFont(F_DIALOG); lblSuffix.setForeground(TEXT_MID);
...
//            }
//            if (!hasError && !sdt.matches("^(03|05|07|08|09)\\d{8}$")) {
//                setFieldError(errSdt, txtSdt, "SĐT không hợp lệ (10 số, bắt đầu 03/05/07/08/09)");
//                txtSdt.requestFocus(); hasError = true;
//            }
//            if (!hasError && !cccd.matches("^09\\d{10}$")) {
//                setFieldError(errCccd, txtCccd, "CCCD phải 12 số, bắt đầu bằng 09");
//                txtCccd.requestFocus(); hasError = true;
//            }
//            if (!hasError && !emailPrefix.isEmpty() && !emailPrefix.matches("^[a-zA-Z0-9._]+$")) {
//                setFieldError(errEmail, txtEmailPrefix, "Email chỉ chứa chữ, số, dấu chấm, gạch dưới");
//                txtEmailPrefix.requestFocus(); hasError = true;
//            }
//            if (hasError) return;
//
//            String email = emailPrefix.isEmpty() ? "" : (emailPrefix + "@gmail.com");
...
//        txtEmail.setText(emailFull.replace("@gmail.com", ""));
//
//        JLabel errTen  = makeErrorLabel();
//        JLabel errSdt  = makeErrorLabel();
//        JLabel errCccd = makeErrorLabel();
...
//                if (!hasErr && !newSdt.matches("^(03|05|07|08|09)\\d{8}$")) { setFieldError(errSdt, txtSdt, "SĐT không hợp lệ"); txtSdt.requestFocus(); hasErr = true; }
//                if (!hasErr && !newCccd.matches("^09\\d{10}$")) { setFieldError(errCccd, txtCccd, "CCCD không hợp lệ (12 số, bắt đầu 09)"); txtCccd.requestFocus(); hasErr = true; }
...
//                if (hasErr) return;
//
//                String newEmail = emailPrefix.isEmpty() ? "" : (emailPrefix + "@gmail.com");
