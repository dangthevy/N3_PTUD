...
//        	    canAccessAdminTools);
	...
	private JPanel createHeader(NhanVien nv) {
		...
		try {
			...
			pnlLeft.add(new JLabel(new ImageIcon(scaledLogo)));
		} catch (Exception e) {
		}

		JLabel lblTitle = new JLabel("HỆ THỐNG QUẢN LÝ BÁN VÉ");
		lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 18));
		lblTitle.setForeground(new Color(0x1E2B3C));
		pnlLeft.add(lblTitle);

		header.add(pnlLeft, BorderLayout.WEST);

		// Header bây giờ trống bên phải, nút Đăng xuất đã nằm trong trang Cá nhân
		return header;
	}
}