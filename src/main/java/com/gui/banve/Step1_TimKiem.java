...
public class Step1_TimKiem extends JPanel {
	...
	private void initUI() {
		...
		setOpaque(false);
		add(UIHelper.createPageTitle("TÌM KIẾM CHUYẾN TÀU", "Nhập thông tin hành trình để bắt đầu"),
				BorderLayout.NORTH);
		...
		pnlSearch.setBorder(BorderFactory.createCompoundBorder(new UIHelper.ShadowBorder(),
		...
		rdoMotChieu.setFont(UIHelper.F_LABEL);
		rdoKhuHoi.setFont(UIHelper.F_LABEL);
		rdoMotChieu.setOpaque(false);
		rdoKhuHoi.setOpaque(false);
		ButtonGroup bg = new ButtonGroup();
		bg.add(rdoMotChieu);
		bg.add(rdoKhuHoi);
		JPanel pnlRadio = new JPanel(new FlowLayout(FlowLayout.LEFT, 20, 0));
		pnlRadio.setOpaque(false);
		pnlRadio.add(rdoMotChieu);
		pnlRadio.add(rdoKhuHoi);
		...
		wrapperGc.weighty = 1.0;
		wrapperGc.anchor = GridBagConstraints.NORTH;
		wrapperGc.insets = new Insets(40, 0, 0, 0);
	...
	private void performSearch() {
		...
		if (!mapGa.containsKey(selDi) || !mapGa.containsKey(selDen)) {
			JOptionPane.showMessageDialog(this, "Vui lòng chọn Nơi Đi và Nơi Đến hợp lệ từ danh sách gợi ý!", "Lỗi",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		if (selDi.equalsIgnoreCase(selDen)) {
			JOptionPane.showMessageDialog(this, "Nơi đi và Nơi đến không được trùng nhau!", "Lỗi",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		if (isRoundTrip) {
			...
			if (sqlNgayVe.compareTo(sqlNgayDi) < 0) {
				JOptionPane.showMessageDialog(this, "Ngày về không được trước ngày đi!", "Lỗi",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
		}

		boolean success = mainTab.getStep2().loadTrainData(gaDi.getMaGa(), gaDen.getMaGa(), sqlNgayDi, isRoundTrip,
				strNgayVe, gaDi.getTenGa(), gaDen.getTenGa());
		if (success) {
	...
	private void applySmartFilterGa(JComboBox<String> cb, String[] fullList) {
		...
		editor.addKeyListener(new java.awt.event.KeyAdapter() {
			...
			public void keyReleased(java.awt.event.KeyEvent e) {
				...
				if (code == KeyEvent.VK_UP || code == KeyEvent.VK_DOWN || code == KeyEvent.VK_ESCAPE
		...
		cb.addActionListener(e -> {
			if ("comboBoxEdited".equals(e.getActionCommand())) {
				...
				for (int i = 0; i < cb.getItemCount(); i++) {
					if (cb.getItemAt(i).equalsIgnoreCase(text)) {
