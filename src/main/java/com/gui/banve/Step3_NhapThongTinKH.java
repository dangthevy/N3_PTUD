...
public class Step3_NhapThongTinKH extends JPanel {
	...
	private JPanel buildBookerCard() {
		...
		chainEnter(txtBkSdt, txtBkHoTen);
		chainEnter(txtBkHoTen, txtBkCccd);
		chainEnter(txtBkCccd, txtBkEmail);
		chainEnter(txtBkEmail, btnConfirmBooker);
		return card;
	}
	...
	// ============================================================
	// BUSINESS LOGIC: GỬI DỮ LIỆU ĐÚNG CÁCH SANG BƯỚC 4
	// ============================================================
	private void confirmBooker() {
		...
		if (confirmedBooker != null && !confirmedBooker.getMaKH().isEmpty()) {
			...
			confirmedBooker.setEmail(fullEmail);
		...
		// [VÁ LỖI]: Bắt buộc phải đẩy dữ liệu Booker vào Map để Step 4 sử dụng
		Map<String, String> bookerMap = new HashMap<>();
		bookerMap.put("maKH", confirmedBooker.getMaKH());
		bookerMap.put("ten", hoTen);
		bookerMap.put("sdt", sdt);
		mainTab.getPassengerDataMap().put("BOOKER", bookerMap);
	...
	private void confirmPassenger() {
		...
		boolean isTreEm = tenLoaiVe.equals("Trẻ em");

		if (hoTen.isEmpty())
			hasError = true;
		if (!isTreEm) {
			if (sdt.isEmpty()) {
				lblPaxSdtError.setText("Người lớn/SV bắt buộc nhập SĐT");
				hasError = true;
			} else if (!sdt.matches("^(03|05|07|08|09)\\d{8}$")) {
				lblPaxSdtError.setText("SĐT phải 10 số hợp lệ");
				hasError = true;
			}
			if (cccd.isEmpty()) {
				lblPaxCccdError.setText("Người lớn/SV bắt buộc nhập CCCD");
				hasError = true;
			} else if (!cccd.matches("^09\\d{10}$")) {
				lblPaxCccdError.setText("CCCD phải 12 số, bắt đầu 09");
				hasError = true;
			}
		}
		if (hasError)
		...
		Map<String, String> currentSeat = mainTab.getSelectedSeatsData().get(currentIdx);
		currentSeat.put("maKH", maKH);
		currentSeat.put("maLoaiVe", maLoaiVe);
		passengerCards.get(currentIdx).setData(hoTen, sdt, cccd, tenLoaiVe);

		// [VÁ LỖI]: Bắt buộc phải đẩy dữ liệu Hành khách vào Map để Step 4 tính tiền
		Map<String, String> paxMap = new HashMap<>();
		paxMap.put("maKH", maKH);
		paxMap.put("hoTen", hoTen);
		paxMap.put("sdt", sdt);
		paxMap.put("loaiVe", maLoaiVe);
		String keyPax = currentSeat.get("maLT") + "_" + currentSeat.get("maCho");
		mainTab.getPassengerDataMap().put(keyPax, paxMap);
	...
	private void applyTitleCase(JTextField tf) {
		((AbstractDocument) tf.getDocument()).setDocumentFilter(new DocumentFilter() {
			public void replace(FilterBypass fb, int offset, int len, String text, AttributeSet a)
					throws BadLocationException {
				if (text != null && !text.isEmpty()) {
					...
					super.replace(fb, offset, len, text, a);
			}
		});
	}
