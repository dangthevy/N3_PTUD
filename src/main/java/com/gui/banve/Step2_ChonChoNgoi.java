...
public class Step2_ChonChoNgoi extends JPanel {
	...
	private class RoutePanel extends JPanel {
		...
		private JToggleButton createSeatBtn(Map<String, Object> gheInfo, String maLT, String maToa) {
			// 1. Chống lỗi NullPointerException
			if (gheInfo == null)
			...
			String trangThai = gheInfo.get("trangThai") != null ? gheInfo.get("trangThai").toString() : "TRONG";

			// 2. Xử lý giá vé (Tách biệt Giá hiển thị UI và Giá thô lưu Session)
			String rawGiaStr = "0";
			...
			if (giaObj != null && !giaObj.toString().isEmpty()) {
				try {
					// Cắt bỏ phần thập phân và ký tự lạ để lấy số nguyên thủy (VD: "500000")
			...
			// Thêm Tooltip để người dùng xem giá khi rê chuột
			b.setToolTipText("Ghế: " + tenCho + " | Giá: " + giaHienThi);

			// 4. Bảng màu Trạng Thái
			Color C_TRONG = new Color(0x2ECC71); // Xanh lá
			Color C_CHON = new Color(0xF39C12); // Cam
			Color C_DADAT = new Color(0xE74C3C); // Đỏ
			Color C_GIUCHO = new Color(0x95A5A6); // Xám nhạt
			Color C_BAOTRI = new Color(0x7F8C8D); // Xám đậm

			boolean isTrong = trangThai.equals("TRONG") || trangThai.equals("SAN_SANG");

			// Kiểm tra xem ghế này đã được người dùng chọn vào giỏ hàng trước đó chưa
			// (Hữu ích khi người dùng qua Step 3 rồi ấn Quay lại Step 2)
			boolean isAlreadySelected = mainTab.getSelectedSeatsData().stream()
					.anyMatch(s -> s.get("maCho").equals(maCho) && s.get("maLT").equals(maLT));

			// 5. Render hình ảnh ban đầu và Khóa nút nếu cần
			if (isAlreadySelected) {
				b.setSelected(true);
				b.setBackground(C_CHON);
				b.setForeground(Color.WHITE);
				b.setBorder(BorderFactory.createLineBorder(C_CHON.darker(), 1));
				b.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
			} else if (isTrong) {
				b.setBackground(C_TRONG);
				b.setForeground(Color.WHITE);
				b.setBorder(BorderFactory.createLineBorder(C_TRONG.darker(), 1));
				b.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
			} else {
				// Ghế đã bị người khác mua, giữ chỗ hoặc đang bảo trì -> KHÓA KHÔNG CHO BẤM
				b.setEnabled(false);
				b.setForeground(Color.WHITE);

				if (trangThai.equals("DADAT")) {
					b.setBackground(C_DADAT);
					b.setBorder(BorderFactory.createLineBorder(C_DADAT.darker(), 1));
					b.setToolTipText("Ghế: " + tenCho + " (Đã đặt)");
				} else if (trangThai.equals("GIUCHO")) {
					b.setBackground(C_GIUCHO);
					b.setBorder(BorderFactory.createLineBorder(C_GIUCHO.darker(), 1));
					b.setToolTipText("Ghế: " + tenCho + " (Đang giữ chỗ)");
				} else {
					b.setBackground(C_BAOTRI);
					b.setBorder(BorderFactory.createLineBorder(C_BAOTRI.darker(), 1));
					b.setToolTipText("Ghế: " + tenCho + " (Bảo trì)");
				}
			}

			// 6. Xử lý sự kiện khi Click (Chỉ hoạt động với ghế TRỐNG)
			...
			b.addActionListener(e -> {
				if (b.isSelected()) {
					...
					seatData.put("giaVe", finalGiaStr); // Truyền giá nguyên thủy
